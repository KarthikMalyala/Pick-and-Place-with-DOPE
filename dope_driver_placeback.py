...
### SYNTHETIC DATA WITH DEEP OBJECT POSE ESTIMATION (DOPE)
### Karthik Malyala
### 12/2/2022
...
class ExampleFullArmMovement:
    ...
    def final_pose(self, x, y, z):
        ...
        req = PlayCartesianTrajectoryRequest()
        req.input.target_pose.x = x - 0.03
        req.input.target_pose.y = y
        req.input.target_pose.z = z 
        req.input.target_pose.theta_x = feedback.base.commanded_tool_pose_theta_x
        req.input.target_pose.theta_y = feedback.base.commanded_tool_pose_theta_y
    ...
    def getCoords(self, msg):
        global target
        #rospy.loginfo("Callback Check")
        obj = PoseStamped()
        obj = msg
        target = obj
    ...
    def main(self):
        ...
        #*******************************************************************************
            ...
            while not rospy.is_shutdown():
                rospy.loginfo("------------------------------------")
                rospy.loginfo("Resetting viewing angle...")
                success &= self.send_pose(0.579, 0, 0.38)
                #success &= self.home()
                rospy.loginfo("------------------------------------")
                print("\n------------------------------------")
                goal = input("Enter your desired object (0 - Green Beans/ 1 - Ketchup/ 2 - Mayo): ")
                print("------------------------------------\n")
                rospy.loginfo("Goal: " + str(goal))
                target = 0
                rospy.loginfo("Subscribing to the dedicated topic...")
                if goal == 0:
                    rospy.loginfo("Subscribing to Green Beans coordinates")
                    rospy.Subscriber('/dope/pose_GreenBeans', PoseStamped, self.getCoords)
                elif goal == 1:
                    rospy.loginfo("Subscribing to Ketchup coordinates")
                    rospy.Subscriber('/dope/pose_Ketchup', PoseStamped, self.getCoords)
                elif goal == 2:
                    rospy.loginfo("Subscribing to Mayo coordinates")
                    rospy.Subscriber('/dope/pose_Mayo', PoseStamped, self.getCoords)
                ...
                if target != 0:
                    try:
                        newPose = listener.transformPose('/base_link', target)
                    ...
                    rospy.loginfo("Destination: " + str(x) + " " + str(y) + " " + str(z))
                    case = 1
                
                elif target == 0:
                    rospy.loginfo("The object that you have selected is not available. Please choose a different one.")

                while(case != 0 and target != 0 and x != 0):
                    ...
                    if case == 1:
                        ...
                        #success &= self.home() 
                        rospy.loginfo("------------------------------------")
                        success &= self.send_pose(0.579, 0, 0.381)
                        rospy.loginfo("------------------------------------")
                        case = 2
                    ...
                    if case == 3:
                        ...
                        diffy = y - cury
                        diffz = z - curz
                        #rospy.loginfo("Destination:" + " " + str(diffx) + " " + str(diffy) + " " + str(diffz))
                        rospy.loginfo("------------------------------------")
                        success &= self.send_pose((curx + (diffx - 0.1)), (cury + (diffy - 0.0)), (curz + (diffz - 0.0)))
                        rospy.loginfo("------------------------------------")
                        #success &= self.send_pose(diffx, diffy, diffz)
                        time.sleep (0.1)
                    ...
                    if case == 4:
                        ...
                        if ((curx <= x + 0.03 and curx >= x - 0.03) and (cury <= y + 0.02 and cury >= y - 0.02) and (curz <= z + 0.02 and curz >= z - 0.02)):
                            case = 5
                    ...
                    # CASE 6: Sends it back home and asks you how you'd like to proceed (ungrip or send back to original position)
                    if case == 6:
                        print("Case 6")
                        time.sleep(0.01)
                        rospy.loginfo("------------------------------------")
                        ...
                        print("\n------------------------------------")
                        ungrip = input("How would you like to proceed? (0 (Ungrip) / 1 (Return the Object to Original Location)) ")
                        print("------------------------------------\n")
                        if (ungrip == 0):
                            success &= self.gripper(0.0)
                            break
                    ...
                    #CASE 7: Sends the object back to its original position and returns the arm to home base
                    if case == 7:
                        print("Case 7")
                        time.sleep(0.01)
                        success &= self.final_pose((x), y, z)
                        if ((curx <= x + 0.03 and curx >= x - 0.03) and (cury <= y + 0.02 and cury >= y - 0.02) and (curz <= z + 0.02 and curz >= z - 0.02)):
                            success &= self.gripper(0.0)
                        rospy.loginfo("------------------------------------")
                        rospy.loginfo("The object has been successfully returned to its original location")
                        rospy.loginfo("Resetting viewing angle...")
                        success &= self.send_pose(0.579, 0, 0.38)
                        #success &= self.home()
                        rospy.loginfo("------------------------------------")
                ...
                if target != 0:
                    ...
                    rospy.loginfo("Looks like the robot didn't find any objects...")
                    print("\n------------------------------------")
                    retry = input("Would you like to try again? (0 (Yes) / 1 (No)) ")
                    print("------------------------------------\n")
                    if (retry == 1):
                        break
                    else:
